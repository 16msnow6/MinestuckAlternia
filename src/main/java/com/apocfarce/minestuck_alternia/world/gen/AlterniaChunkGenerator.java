...
public class AlterniaChunkGenerator extends AbstractChunkGenerator<AlterniaGenSettings> {
   ...
   public AlterniaChunkGenerator(IWorld worldIn, BiomeProvider provider, AlterniaGenSettings settingsIn) {
      ...
      settings = settingsIn;
      settings.setDefautBlock(AlterniaBlocks.redRock.getDefaultState());;
      defaultBlock = this.settings.getDefaultBlock();
      defaultFluid = this.settings.getDefaultFluid();
         ...
         for(int l = 0; l < 5; ++l) {
            ...
            for(int j1 = -2; j1 <= 2; ++j1) {
               for(int k1 = -2; k1 <= 2; ++k1) {
                  Biome biome1 = p_202108_1_[(k + j1 + 2 + (l + k1 + 2) * 10)%p_202108_1_.length];
